...
for i in range(1, len(self.hidden_layers)):
	network.append(_layer(self.hidden_layers[i-1], self.hidden_layers[i]))	network.append(_layer(self.hidden_layers[-1], self.output_dim))
return network
...
print (x_in)
    ...
    def _backward_pass(self, yhot):
        transfer_derivative = self._relu_derivative # sig' = f(sig)
...
# relu derivative
def _relu_derivative(self, relu):
